...
class _EmployeeAttendanceManagementScreenState
  ...
  Future<void> _searchWorkers(String query) async {
    ...
    try {
      // Real API call to search workers
      final appState = context.read<AppState>();
      final response = await http.get(
        Uri.parse(
            'https://dhruvbackend.vercel.app/api/workers/search?name=${Uri.encodeComponent(query)}'),
        headers: {
          'Content-Type': 'application/json',
          if (appState.service.authToken != null)
        ...
        setState(() {
          _searchResults = workersData
              .map((worker) => {
                    ...
                    'totalShifts': _safeInt(worker['totalShifts']),
                    'completedHours': _safeDouble(worker['completedHours']),
                    'averageRating': _safeDouble(worker['rating']),
                  })
              .toList();
          _isSearching = false;
        });
  ...
  Future<void> _handleEditHours(AttendanceRecord record) async {
    ...
    if (result != null && mounted) {
      final appState = context.read<AppState>();
      try {
        // Debug: Check if record.id is valid
        print('🔍 Attempting to update hours for record ID: "${record.id}"');
        print(
            '🔍 Record details: workerId=${record.workerId}, jobId=${record.jobId}');

        if (record.id.isEmpty) {
          throw Exception('Invalid record: Attendance ID is empty');
        }
  ...
  Widget _buildSummaryCards(
    ...
    return Column(
      children: [
        // First row: Total Workers and Completed
        Row(
          children: [
            Expanded(
              child: _SummaryCard(
        ...
        // Second row: Total Hours and Total Payroll
        Row(
          children: [
            Expanded(
              child: _SummaryCard(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            Column(
              children: [
                // First row: Scheduled and Earnings
                Row(
                  children: [
                    Expanded(
                      child: _InfoTile(
                ...
                const SizedBox(height: 8),
                // Second row: Clock In and Clock Out
                Row(
                  children: [
                    Expanded(
                      child: _InfoTile(
